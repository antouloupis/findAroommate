...
def search_listings(request):
    ...
    context = {
        'query': search_query,
        'listings': listings,
        'filter_form': filter_form,  # Pass the filter form for use in the modal
    }
...
def filter_listings(request):
    ...
    if filters.is_valid():
        ...
        # Access form fields using `cleaned_data`
...
def search_users(request):
    user_form = UserForm(request.GET or None)  # Instantiate the form
    users = Profiles.objects.filter(hidden=False)  # Start with all visible listings